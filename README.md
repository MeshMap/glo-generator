...
- Four prompt types with color coding, mixing in some NYC flavor
  ...
  - **Today** (purple) — a drawable idea tied to today's date, pulled from
    a curated list of fun/notable holidays and monthly observances (e.g.
    "It's Pride Month — a rainbow-striped heart"). Offline and static, in
    the spirit of timeanddate.com/holidays/fun, not a live feed
...
Item/Tag/Mural prompt lists live in `src/prompts.js`. The Today mode's fun
...
`fallback` list for undated days. Add, remove, or reword any of it freely —
...
└── src/
    ...
    ├── holidays.js   # Today mode's fun-holiday dataset (edit these)
